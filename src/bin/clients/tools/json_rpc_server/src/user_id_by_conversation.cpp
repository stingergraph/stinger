...
int64_t 
JSON_RPC_user_id_by_conversation::operator()(rapidjson::Value * params, rapidjson::Value & result, rapidjson::MemoryPoolAllocator<rapidjson::CrtAllocator> & allocator)
{
  int64_t source;
  bool strings;
  ...
  for (int64_t i = 0; i < nv; i++) {
    STINGER_FORALL_EDGES_OF_TYPE_OF_VTX_BEGIN(S, tweets_etype, i) {
      ...
      if (conversation_id == this_component_id) {
	name.SetInt64(i);
